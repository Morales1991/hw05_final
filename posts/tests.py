...
class HomeWorkTest(TestCase):
    ...
    def test_user_createPost(self):
        ...
        self.assertTrue(
            Post.objects.filter(text='test', author=self.test_user).exists()
        )
    ...
    def test_anonim_user(self):
        ...
        self.assertRedirects(
            response, '/auth/login/?next=/new/', status_code=302, target_status_code=200,
    ...
    def test_edit_post(self):
        ...
        response = self.client.post(
            f'/{self.test_user.username}/{post.id}/edit/', {'text': new_text}
        )
        self.assertRedirects(
            response, f'/{self.test_user.username}/{post.id}/', status_code=302, 
    ...
    def test_img(self):
        post = Post.objects.create(
            author=self.test_user, group=self.test_group,  text='еще какой то текст',
    ...
    def test_fake_img(self):
        img=SimpleUploadedFile(name='test_image.txt', content=b'test file content', content_type='text/plain')
        response = self.client.post('/new/', {'text': 'ложная картинка', 'image': img})
        self.assertFormError(
            response, 'form', 'image', 'Загрузите правильное изображение. Файл, который вы загрузили,'
    ...
    def test_follow(self):
        self.client.get(f'/{self.test_user_2.username}/follow/')
        count = Follow.objects.filter(author=self.test_user_2, user=self.test_user).count()
        self.assertEqual(count, 1)

        self.client.get(f'/{self.test_user_2.username}/unfollow/')
        count = Follow.objects.filter(author=self.test_user_2, user=self.test_user).count()
    ...
    def test_comment(self):
        post = Post.objects.create(author=self.test_user, text='пост для коммента')
        
        self.client.post(f'/{self.test_user.username}/{post.id}/comment/', {'text': 'test'})
        self.assertTrue(
            Comment.objects.filter(text='test', author=self.test_user).exists()
        )
        
        self.client.logout()
        response = self.client.get(f'/{self.test_user.username}/{post.id}/comment/')
        
        self.assertRedirects(
            response, '/auth/login/?next=/'f'{self.test_user.username}/1/comment/', status_code=302, 
            target_status_code=200, msg_prefix='', fetch_redirect_response=True
        )
